...
# CONSTRAINTS
    ...
    # Energy
    e_buy = dual_model.addConstr(-p_flow - max_grid <= 0, name='e_buy') # Initially = data.Prices
    ...
    m_h2_elec = dual_model.addConstr(elec_op*data.Electrolyzer[0] + compressor_h2 <= 0, name='m_h2_elec')
    ...
    m_h2_sto_dis = dual_model.addConstr(h2_storage/data.H2storage[1] - C_rate_h2_2[:-1] - reac_H2[:-1] <= 0, name='m_h2_sto_dis')
    m_h2_sto_cha_h = dual_model.addConstr(- C_rate_h2_1[data.hours-1] - compressor_h2[data.hours-1] <= 0, name='m_h2_sto_cha_h')
    m_h2_sto_dis_h = dual_model.addConstr(- C_rate_h2_2[data.hours-1] - reac_H2[data.hours-1] <= 0, name='m_h2_sto_dis_h')
    ...
    m_CO2_buy = dual_model.addConstr(CO2_str_min - CO2_str_max + CO2_comp <= 0, name='m_CO2_buy') # Initially: = data.CO2_prices
    m_CO2_sto_cha = dual_model.addConstr(- CO2_sto*data.CO2storage[0] - CO2_comp[:-1] <= 0, name='m_CO2_sto_cha')
    m_CO2_sto_dis = dual_model.addConstr(CO2_sto/data.CO2storage[1] - reac_CO2[:-1] <= 0, name='m_CO2_sto_dis')
    m_CO2_sto_cha_h = dual_model.addConstr(- CO2_comp[data.hours-1] <= 0, name='m_CO2_sto_cha_h')
    m_CO2_sto_dis_h = dual_model.addConstr( - reac_CO2[data.hours-1] <= 0, name='m_CO2_sto_dis_h')

    # Methanol reactor
    m_h2_reac = dual_model.addConstr(- compressor_h2 - reac_H2 <= 0, name='m_h2_reac')
    m_CO2_reac = dual_model.addConstr(- CO2_comp - reac_CO2 <= 0, name='m_CO2_reac') 
    m_rawMeOH_reac_0 = dual_model.addConstr(-reac_max[0] + reac_min[0] + reac_H2[0]*data.Reactor[0] + reac_CO2[0]*data.Reactor[1] + Reac_ramp_up[0] - Reac_ramp_down[0] + compr_MeOH[0] <= 0, name='m_rawMeOH_reac_0')
    m_rawMeOH_reac = dual_model.addConstr(-reac_max[1:-1] + reac_min[1:-1] + reac_H2[1:-1]*data.Reactor[0] + reac_CO2[1:-1]*data.Reactor[1] - Reac_ramp_up[:-1] + Reac_ramp_down[:-1] + Reac_ramp_up[1:] - Reac_ramp_down[1:] + compr_MeOH[1:-1] <= 0, name='m_rawMeOH_reac')
    m_rawMeOH_reac_h = dual_model.addConstr(-reac_max[data.hours-1] + reac_min[data.hours-1] + reac_H2[data.hours-1]*data.Reactor[0] + reac_CO2[data.hours-1]*data.Reactor[1] - Reac_ramp_up[data.hours-2] + Reac_ramp_down[data.hours-2] + compr_MeOH[data.hours-1] <= 0, name='m_rawMeOH_reac_h')
    ...
    m_rawMeOH_sto_cha = dual_model.addConstr(-rawMeOH_sto*data.raw_MeOH_sto[0] - compr_MeOH[:-1] <= 0, name='m_rawMeOH_sto_cha')
    m_rawMeOH_sto_dis = dual_model.addConstr(+rawMeOH_sto/data.raw_MeOH_sto[1] - dest_rawMeOH[:-1] <= 0, name='m_rawMeOH_sto_dis')
    m_rawMeOH_sto_cha_h = dual_model.addConstr( - compr_MeOH[data.hours-1] <= 0, name='m_rawMeOH_sto_cha')
    m_rawMeOH_sto_dis_h = dual_model.addConstr( - dest_rawMeOH[data.hours-1] <= 0, name='m_rawMeOH_sto_dis')

    # Methanol distilator
    m_rawMeOH_dest = dual_model.addConstr(- compr_MeOH - dest_rawMeOH <= 0, name='m_rawMeOH_dest')
    m_MeOH_dest_0 = dual_model.addConstr(- dest_max[0] + dest_min[0] + dest_rawMeOH[0]*data.Destilator[0] + dest_sto[0] + dest_ramp_up[0] - dest_ramp_down[0] <= 0, name='m_MeOH_dest_0')
    m_MeOH_dest = dual_model.addConstr(- dest_max[1:-1] + dest_min[1:-1] + dest_rawMeOH[1:-1]*data.Destilator[0] + dest_sto[1:-1] - dest_ramp_up[:-1] + dest_ramp_down[:-1] + dest_ramp_up[1:] - dest_ramp_down[1:] <= 0, name='m_MeOH_dest')
    m_MeOH_dest_h = dual_model.addConstr(- dest_max[data.hours-1] + dest_min[data.hours-1] + dest_rawMeOH[data.hours-1]*data.Destilator[0] + dest_sto[data.hours-1] - dest_ramp_up[data.hours-2] + dest_ramp_down[data.hours-2] <= 0, name='m_MeOH_dest_h')
    ...
    m_MeOH_cha = dual_model.addConstr(- dest_sto[:-1] - pure_sto*data.Pure_MeOH_sto[0] <= 0, name='m_MeOH_cha')
    m_MeOH_dis = dual_model.addConstr(pure_sto/data.Pure_MeOH_sto[1] + balance[:-1] <= 0, name="m_MeOH_dis") 
    m_MeOH_cha_h = dual_model.addConstr(- dest_sto[data.hours-1] <= 0, name='m_MeOH_cha_h')
    m_MeOH_dis_h = dual_model.addConstr(+ balance[data.hours-1] <= 0, name="m_MeOH_dis_h") 

    q_constr = dual_model.addConstr(
    1 == 
 ...
 # OBJECTIVE FUNCTION
    ...
    dual_model.setObjective(1, GRB.MAXIMIZE)

    # Solve dual model
    # dual_model.setParam("OutputFlag", 0)
    # dual_model.setParam('Method', 2)  # Barrier method
    # dual_model.setParam('Crossover', 0)
    # dual_model.setParam('MIPGap', 0.001)  # Allow a 0.001% gap
    # dual_model.setParam('FeasibilityTol', 1e-7)  # Default is 1e-6
    # dual_model.setParam("NumericFocus", 1)  # Options: 0 (default), 1, 2, or 3 model too slow
    # dual_model.setParam('ScaleFlag', 1)  # Enable automatic scaling (default)

    dual_model.optimize()
    # print(dual_model.getAttr("Kappa"))  # Condition number of the matrix
    ...
    if dual_model.Status == GRB.OPTIMAL:
        ...
        duals = Sub_Results(
            ...
            C_raw_MeOH_1= 0, #C_raw_MeOH_1.X,
            C_raw_MeOH_2= 0,#C_raw_MeOH_2.X,
            dest_min=dest_min.X,
            dest_max=dest_max.X,
            ...
            MeOH_min=MeOH_min.X,
            MeOH_max=MeOH_max.X,
            C_rate_MeOH_1= 0,#C_rate_MeOH_1.X,
            C_rate_MeOH_2= 0,#C_rate_MeOH_2.X,
            balance=balance.X,
        )

        return objective_val, duals
